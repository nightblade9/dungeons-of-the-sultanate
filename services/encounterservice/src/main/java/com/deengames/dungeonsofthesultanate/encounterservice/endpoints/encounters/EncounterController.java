...
public class EncounterController {
    ...
    public JSONObject tryEncounter(@RequestBody JSONObject body) throws Exception {
        ...
        var leveledUpResults = client.post(statsServiceUrl, null, JSONObject.class);
        if ((int)leveledUpResults.get("levels_gained") > 0) {
            var levelUpLogs = (ArrayList<String>)leveledUpResults.get("logs");
            logs.addAll(levelUpLogs);
        }
